...
def main(input_path, output_path, prompt_path, column_name, model):
    prompts = load_yaml_config(paths["constraint_prompts"])
    prompt_template = prompts.get("characteristics_to_list", "")
    ...
    df[column_name + "_List"] = df[column_name].apply(
        lambda x: convert_text_to_list(x, prompt_template, model)
    )
